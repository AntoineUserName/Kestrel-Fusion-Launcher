...
/**
 * 
 * Does not edit the file if no backups are found.
 * 
 * @param {string} filegamepath 
 * @param {undefined|(datas)=>"new file datas"} dataconstructor 
 * @param {undefined|{init:fs.ReadSyncOptions, new: fs.WriteFileOptions}} fileoptions 
 * @returns 
 */
function loadBackupFile(filegamepath, dataconstructor, fileoptions) {
    ...
    try {
        ...
        let dirPathB = path.join( config['game-location'],
            path.dirname(filegamepath) // Get the dir path
        );
        
        if(fs.existsSync(dirPathB) == false) {
            fs.mkdirSync( dirPathB, { recursive: true }); // Write folder that not exist
        }
